...
The Slipway source repository may track narrowly scoped Codex contributor tooling at root `AGENTS.md` and `.codex/` to coordinate work on that source tree. This is not Slipway behavior, a host installation artifact, or a distribution input: unrelated projects neither install nor require it, and the eight-skill suite remains unchanged.
...
54. As a developer, I want Slipway to load baseline private context before it explores my repository, while warning and using clearly labeled fallback tools for read-only work when context is unavailable, so that project guidance is normally applied without turning a local context outage into a total read-only outage.
...
- Keep one canonical private context registry on the ledger. Version it by the context directory's Git tree ID and cache it only under ignored `.slipway-local/context/`. Attempt bootstrap and load `operations: [all]` coordinator modules before classification or repository discovery, then load additional required modules before operation-specific action. Never overwrite divergent local context. Warn and permit a clearly labeled, response-only degraded fallback for read-only work; it creates no workflow state and cannot satisfy review or delivery gates. Block mutation and delivery workflow until required context is healthy, except for explicitly confirmed setup initialization or repair. Do not create a tracked public extension contract.
