...
4. Only after that clean fast-forward, resolve the canonical private-context tree, validate it, and refresh/verify the affected Repo-B agentic-main worktree's ignored context cache. Do not copy context into main history.
...
6. For other active work branches, record the new baseline and open an explicit merge/rebase reconciliation gate according to project policy. Do not silently rewrite or merge into them. Re-check their private context before resumed work.
