...
2. Locate the machine binding and ledger worktree described in [store.md](references/store.md). If setup is missing, incomplete, stale, or inconsistent, invoke `$slipway-setup` and stop lane execution.
...
- Keep product work on the agentic work branch. Keep agentic metadata in separate commits. Treat only exact reviewed product-only commits as delivery cargo.
...
- During setup, copy [project.md](assets/project.md), [preferences.md](assets/preferences.md), [portfolio.md](assets/portfolio.md), and the machine-local [binding.md](assets/binding.md).
