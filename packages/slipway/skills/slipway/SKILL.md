...
4. Resolve the private context modules for that operation. For an operation that classifies, load every coordinator-targeted entrypoint before classification; for a forced direct operation, load them before its first action. Forced status only validates and reports context and does not activate modules. For every module-selecting operation, record activated and skipped modules and block on missing required context or a conflict with core policy.
