...
5. Before any lane work, apply the complete [private context lifecycle](store.md#private-context-lifecycle). Validate and safely cache the exact ledger context tree under ignored `.slipway-local/context/`, preserve unrelated clone-local exclusions, resolve the modules for this operation, and load every coordinator-targeted entrypoint. Do not start the lane until required context is healthy and loaded; never overwrite divergent cached bytes automatically.
