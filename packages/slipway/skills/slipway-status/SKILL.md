...
Invoke `$slipway` with the forced read-only `status` operation and the user's scope. Do not classify it as development or require optional lane capabilities. Apply the shared [overlay lifecycle status contract](../slipway/references/store.md#overlay-lifecycle), then report state counts, verified run facts, open gates, PR states, and one next action per run. Status must not invoke `$slipway-setup`, execute lane work, rewrite portfolio state, or require delivery capabilities.
