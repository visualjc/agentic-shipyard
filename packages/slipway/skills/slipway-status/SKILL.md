...
Invoke `$slipway` with the forced read-only `status` operation and the user's scope. Do not classify it as development or require optional lane capabilities. Apply the read-only [private context lifecycle](../slipway/references/store.md#private-context-lifecycle), then report state counts, verified run facts, context observations, open gates, PR states, and one next action per run. Status must not invoke `$slipway-setup`, hydrate context, activate lane modules, execute lane work, rewrite portfolio state, or require delivery capabilities.
