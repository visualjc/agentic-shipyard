...
unconditional placeholder. The top-level `syncFreshness` field records these
sync facts independently from optional graph acceleration.
...
`graphFreshness` plus its enabled flag, pinned adapter receipt,
...
a dependency. It only snapshots Git source, an existing descriptor, and an
existing external lock record. A disabled or non-required graph does not
replace sync freshness, blockers, or the sync next action. Contributors append
blockers in priority order; once a blocker establishes a next action, a later
conservative contributor may add evidence but cannot erase that action. An
enabled non-fresh graph selects `inspect-source-directly` only when no earlier
blocker controls the action. A graph is never delivery authority.
