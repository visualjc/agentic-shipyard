...
test("the public factory runs the pinned, locked, child-process review path end to end",async()=>{
  ...
  try {
    ...
    const observed=JSON.parse(await readFile(join(home,".fixture-observed.json"),"utf8"));assert.equal(observed.bundle.records.acceptance,acceptance);assert.equal(observed.bundle.records.manifest,manifestBytes);for(const forbidden of [root,envelopePath,`deliveries/${deliveryId}/evidence/acceptance.json`])assert.equal(JSON.stringify(observed.bundle).includes(forbidden),false);assert.equal(observed.reviewTarget,"pinned code");assert.equal(JSON.parse(records[`deliveries/${deliveryId}/evidence/review-result-r.json`]).process.bundleDigest,result.process.bundleDigest);
