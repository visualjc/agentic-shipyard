...
const request={schemaVersion:1 as const,issueId,productSha:sha,reviewId:"r",reviewerEnvelopePath:"reviewer.json",intentRefs:[`${prefix}/intent.md`],evidenceRefs:[`${prefix}/evidence/acceptance.json`]};
const envelope:any={deliveryId,role:"reviewer",host:"codex",productSha:sha,evidenceManifestDigest:manifestDigest,adapter:{envelopePath:"reviewer.json",repoRoot:"/repo"}};
...
test("trusted dispatch loads and seals canonical pinned manifest and acceptance bytes before adapter invocation",async()=>{let loaded=0,called=0,bundle:any;const bridge=new TrustedReviewDispatcher({load:async()=>{loaded++;return {envelope,records};}} as any,{codex:{review:async(dispatch:any)=>{called++;bundle=JSON.parse(dispatch.sealedBundle);return {result:{},attestation:{}} as any;}}});await bridge.dispatch(envelope,requestPath,request);assert.equal(loaded,1);assert.equal(called,1);assert.deepEqual(bundle.records,records);assert.equal(bundle.records[`${prefix}/evidence/acceptance.json`],acceptanceBytes);assert.equal(bundle.request.issueId,issueId);});
