...
test("experimental graph documentation and package policy retain the reviewed safety contract", async () => {
  ...
  assert.match(graphify, /code-only/i); assert.match(graphify, /GRAPHIFY_OUT/); assert.match(graphify, /--out/); assert.match(graphify, /query logging/i); assert.match(graphify, /whole-product-tree/i); assert.match(graphify, /arbitrary created, removed, or modified paths/i); assert.match(graphify, /Pre-existing paths, and new paths whose identity or\s*content changes again before cleanup, are never deleted/i); assert.match(graphify, /no code, query, image,\s*or document may be sent\s*to a provider/i);
