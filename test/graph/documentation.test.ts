...
test("experimental graph documentation and package policy retain the reviewed safety contract", async () => {
  ...
  assert.match(graphify, /code-only/i); assert.match(graphify, /GRAPHIFY_OUT/); assert.match(graphify, /--out/); assert.match(graphify, /query logging/i); assert.match(graphify, /whole-product-tree/i); assert.match(graphify, /arbitrary created, removed, or modified paths/i); assert.match(graphify, /exact top-level reviewed Graphify leak\s*roots `graphify-out` and `\.graphify`/i); assert.match(graphify, /arbitrary additions anywhere else are\s*ownership-ambiguous, are preserved/i); assert.match(graphify, /restored only when a final\s*whole-tree observation exactly matches the before observation/i); assert.match(graphify, /no code, query, image,\s*or document may be sent\s*to a provider/i);
