...
const artifact = "9".repeat(64), content = "c".repeat(64);
const receipt = { executable: "/tools/graphify", version: "0.9.32", sourceReceipt: GRAPHIFY_RECEIPT, artifactSha256: artifact };
...
test("Graphify is disabled before observing, locking, or writing", async () => {
  let calls = 0; const result = await refreshGraphify(source, { enabled: false, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", lock, sourceReader: reader, command: { ...command, observe: async () => { calls++; return receipt; } }, files: { canonicalPath: async p => p, exists: async () => true, observeProductTree: async () => ({}), auditProductTree: async () => ({ state: "clean" }), contentDigest: async () => content } });
...
test("Graphify holds one lock through command, audit, source reread and descriptor", async () => {
  const trace: string[] = []; const result = await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", lock: { acquire: async () => { trace.push("acquire"); return { lock: { ownerHost: "x", ownerPid: 1, acquiredAt: "2026-01-01T00:00:00Z" } }; }, release: async () => { trace.push("release"); } }, sourceReader: { ...reader, worktreeStatus: async () => { trace.push("reread"); return ""; } }, command: { ...command, observe: async () => { trace.push("observe"); return receipt; }, run: async () => { trace.push("run"); return { code: 0, stdout: "", stderr: "", timedOut: false }; } }, files: { canonicalPath: async p => p, exists: async () => { trace.push("exists"); return true; }, observeProductTree: async () => { trace.push("audit-before"); return {}; }, auditProductTree: async () => { trace.push("audit-after"); return { state: "clean" }; }, contentDigest: async () => { trace.push("digest"); return content; } } });
  assert.equal(result.decision.state, "fresh"); assert.deepEqual(trace, ["acquire", "observe", "audit-before", "run", "exists", "audit-after", "digest", "reread", "release"]);
});
test("Graphify rejects echo-style/malformed observation, missing lock, release failure, and source drift", async () => {
  const base = { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async (p: string) => p, exists: async () => true, observeProductTree: async () => ({}), auditProductTree: async () => ({ state: "clean" }), contentDigest: async () => content } };
  assert.equal((await refreshGraphify(source, base)).decision.state, "invalid");
  assert.equal((await refreshGraphify(source, { ...base, lock, command: { ...command, observe: async () => ({ executable: "/tools/graphify", version: "0.9", sourceReceipt: "wrong", artifactSha256: artifact }) } })).decision.state, "unavailable");
...
test("Graphify redacts throwing canonicalization, acquire, file, and command ports", async () => {
  const common = { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async (p: string) => p, exists: async () => true, observeProductTree: async () => ({}), auditProductTree: async () => ({ state: "clean" }), contentDigest: async () => content }, lock };
  const cases = [
    ...
    { ...common, files: { ...common.files, auditProductTree: async () => { throw new Error("file-secret"); } } },
...
test("Graphify completes its product-tree audit even when the command fails", async () => {
  const trace: string[] = [], result = await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command: { ...command, run: async () => { trace.push("run"); throw new Error("command-private"); } }, files: { canonicalPath: async path => path, exists: async () => true, observeProductTree: async () => { trace.push("before"); return {}; }, auditProductTree: async () => { trace.push("after"); return { state: "restored" }; }, contentDigest: async () => content }, lock });
  assert.equal(result.decision.state, "failed"); assert.deepEqual(trace, ["before", "run", "after"]); assert.doesNotMatch(result.decision.reason, /private/);
});

test("release failure rolls back a descriptor staged while the transaction lock was held", async () => {
  const trace: string[] = []; const result = await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async p => p, exists: async () => true, observeProductTree: async () => ({}), auditProductTree: async () => ({ state: "clean" }), contentDigest: async () => content }, lock: { acquire: async () => { trace.push("acquire"); return { lock: { ownerHost: "x", ownerPid: 1, acquiredAt: "2026-01-01T00:00:00Z" } }; }, release: async () => { trace.push("release"); throw new Error("release-private"); } }, descriptorPublisher: { write: async () => { trace.push("descriptor-write"); }, remove: async () => { trace.push("descriptor-remove"); } } });
...
test("a blocked lock acquisition never removes another transaction's descriptor", async () => {
  let removed = 0;
  const result = await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async p => p, exists: async () => true, observeProductTree: async () => ({}), auditProductTree: async () => ({ state: "clean" }), contentDigest: async () => content }, lock: { acquire: async () => ({ decision: { state: "blocked", authoritative: false, fallbackAction: "inspect-source-directly", reason: "held" } }), release: async () => { throw new Error("must not release"); } }, descriptorPublisher: { write: async () => { throw new Error("must not write"); }, remove: async () => { removed++; } } });
...
test("Graphify lock record stays in the sibling external state root", async () => {
  let path = ""; await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async p => p, exists: async () => true, observeProductTree: async () => ({}), auditProductTree: async () => ({ state: "clean" }), contentDigest: async () => content }, lock: { acquire: async value => { path = value; return { lock: { ownerHost: "x", ownerPid: 1, acquiredAt: "2026-01-01T00:00:00Z" } }; }, release: async () => undefined } });
...
test("one physical worktree cache keeps the same operation lock across commits", async () => {
  ...
  for (const current of [source, next]) {
    ...
    const result = await refreshGraphify(current, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: currentReader, command, files: { canonicalPath: async p => p, exists: async () => true, observeProductTree: async () => ({}), auditProductTree: async () => ({ state: "clean" }), contentDigest: async () => content }, lock: { acquire: async path => { paths.push(path); return { lock: { ownerHost: "x", ownerPid: 1, acquiredAt: "2026-01-01T00:00:00Z" } }; }, release: async () => undefined } });
