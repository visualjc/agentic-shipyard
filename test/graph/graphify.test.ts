...
test("Graphify holds one lock through command, audit, source reread and descriptor", async () => {
  const trace: string[] = []; const result = await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", lock: { acquire: async () => { trace.push("acquire"); return { lock: { ownerHost: "x", ownerPid: 1, acquiredAt: "2026-01-01T00:00:00Z" } }; }, release: async () => { trace.push("release"); } }, sourceReader: { ...reader, worktreeStatus: async () => { trace.push("reread"); return ""; } }, command: { ...command, observe: async () => { trace.push("observe"); return receipt; }, run: async () => { trace.push("run"); return { code: 0, stdout: "", stderr: "", timedOut: false }; } }, files: { canonicalPath: async p => p, exists: async () => { trace.push("exists"); return true; }, productGraphifyLeak: async () => { trace.push("audit"); return false; }, contentDigest: async () => { trace.push("digest"); return content; } } });
  assert.equal(result.decision.state, "fresh"); assert.deepEqual(trace, ["acquire", "observe", "run", "audit", "exists", "digest", "reread", "release"]);
});
test("Graphify rejects echo-style/malformed observation, missing lock, release failure, and source drift", async () => {
  const base = { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async (p: string) => p, exists: async () => true, productGraphifyLeak: async () => false, contentDigest: async () => content } };
...
test("Graphify redacts throwing canonicalization, acquire, file, and command ports", async () => {
  const common = { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async (p: string) => p, exists: async () => true, productGraphifyLeak: async () => false, contentDigest: async () => content }, lock };
  const cases = [
...
test("release failure rolls back a descriptor staged while the transaction lock was held", async () => {
  const trace: string[] = []; const result = await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async p => p, exists: async () => true, productGraphifyLeak: async () => false, contentDigest: async () => content }, lock: { acquire: async () => { trace.push("acquire"); return { lock: { ownerHost: "x", ownerPid: 1, acquiredAt: "2026-01-01T00:00:00Z" } }; }, release: async () => { trace.push("release"); throw new Error("release-private"); } }, descriptorPublisher: { write: async () => { trace.push("descriptor-write"); }, remove: async () => { trace.push("descriptor-remove"); } } });
...
test("a blocked lock acquisition never removes another transaction's descriptor", async () => {
  let removed = 0;
  const result = await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async p => p, exists: async () => true, productGraphifyLeak: async () => false, contentDigest: async () => content }, lock: { acquire: async () => ({ decision: { state: "blocked", authoritative: false, fallbackAction: "inspect-source-directly", reason: "held" } }), release: async () => { throw new Error("must not release"); } }, descriptorPublisher: { write: async () => { throw new Error("must not write"); }, remove: async () => { removed++; } } });
...
test("Graphify lock record stays in the sibling external state root", async () => {
  let path = ""; await refreshGraphify(source, { enabled: true, localOnlyApproved: true, reviewedToolSource: GRAPHIFY_RECEIPT, artifactSha256: artifact, cacheRoot: "/cache/wt", executablePath: "/tools/graphify", sourceReader: reader, command, files: { canonicalPath: async p => p, exists: async () => true, productGraphifyLeak: async () => false, contentDigest: async () => content }, lock: { acquire: async value => { path = value; return { lock: { ownerHost: "x", ownerPid: 1, acquiredAt: "2026-01-01T00:00:00Z" } }; }, release: async () => undefined } });
