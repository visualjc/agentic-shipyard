...
import { buildCanonicalReviewerBundle, reviewerBundleDigest } from "../../src/review/bundle.js";

const productSha="a".repeat(40),head="f".repeat(40),reviewedHead="e".repeat(40),at="2026-08-04T00:00:00.000Z",deliveryId="d",prefix=`deliveries/${deliveryId}/evidence/`,intentPath=`deliveries/${deliveryId}/intent.md`,acceptancePath=`${prefix}acceptance.json`;
...
const request=(reviewId:string,issueId=manifest.issueId)=>({schemaVersion:1 as const,issueId,productSha,reviewId,reviewerEnvelopePath:"reviewer.json",intentRefs:[intentPath],evidenceRefs:[acceptancePath],reviewedLedgerSha:reviewedHead,manifestDigest,acceptanceDigest:createHash("sha256").update(canonicalJson(acceptance)).digest("hex")});
const result=(reviewId:string,findings:any[]=[],time=at,reviewedAcceptance=acceptance)=>{const reviewRequest={...request(reviewId),acceptanceDigest:createHash("sha256").update(canonicalJson(reviewedAcceptance)).digest("hex")},bundle=buildCanonicalReviewerBundle(reviewRequest,{intent:"proof",manifest:manifestBytes,acceptance:canonicalJson(reviewedAcceptance),instructions:"proof"});return {schemaVersion:1 as const,reviewId,productSha,reviewer:"codex",startedAt:time,finishedAt:time,process:{processId:1,sessionId:`s-${reviewId}`,fresh:true as const,commandVersion:"codex-test-1",bundleDigest:reviewerBundleDigest(bundle)},findings,successful:true};};
...
function fixture(entries:readonly ReturnType<typeof entry>[],overrides:Readonly<{product?:()=>Promise<string>;scope?:()=>Promise<Scope>;acceptance?:typeof acceptance;reviewedIntent?:string;currentIntent?:string;omitReviewedInstructions?:boolean}>= {}){
  let inventoryReads=0;
  const currentAcceptance=overrides.acceptance??acceptance,cited=new Set([intentPath,`deliveries/${deliveryId}/review.json`,`${prefix}manifest.json`,acceptancePath,...currentAcceptance.items.flatMap(item=>item.evidenceRefs),"finding/f-1.txt","resolution/f-1.txt"]);
  const ledger={objectFormat:async()=>"sha1" as const,snapshot:async()=>({head,records:{}}),currentInventory:async(received:string)=>{inventoryReads++;assert.equal(received,prefix);return {head,entries};},read:async(ledgerSha:string,paths:readonly string[])=>Object.fromEntries(paths.filter(path=>cited.has(path)&&!(ledgerSha===reviewedHead&&overrides.omitReviewedInstructions&&path===`deliveries/${deliveryId}/review.json`)).map(path=>[path,path===`${prefix}manifest.json`?manifestBytes:path===acceptancePath?canonicalJson(currentAcceptance):path===intentPath?(ledgerSha===reviewedHead?overrides.reviewedIntent??"proof":overrides.currentIntent??"proof"):"proof"])),transact:async()=>head};
...
test("a same-SHA acceptance renewal stales A1 review, while a renewed A2 request/result is eligible",async()=>{
  ...
  const renewed=fixture([entry("manifest.json",manifest,1),entry("acceptance.json",a2,5),entry("review-request-a2.json",a2Request,6),entry("review-result-a2.json",result("a2",[],at,a2),7)],{acceptance:a2});
  assert.equal((await renewed.gate.evaluate()).promotionEligible,true);
});

test("promotion rejects forged bundle digests and changed or missing pinned reviewer records",async()=>{
  const reviewRequest=request("bundle"),validResult=result("bundle"),entries=(reviewResult:ReturnType<typeof result>)=>baseline(entry("review-request-bundle.json",reviewRequest,3),entry("review-result-bundle.json",reviewResult,4));
  await assert.rejects(fixture(entries({...validResult,process:{...validResult.process,bundleDigest:"b".repeat(64)}})).gate.evaluate());
  await assert.rejects(fixture(entries(validResult),{reviewedIntent:"old intent",currentIntent:"new intent"}).gate.evaluate());
  await assert.rejects(fixture(entries(validResult),{omitReviewedInstructions:true}).gate.evaluate());
  assert.equal((await fixture(entries(validResult)).gate.evaluate()).promotionEligible,true);
});
