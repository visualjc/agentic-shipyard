...
const request=(reviewId:string,issueId=manifest.issueId)=>({schemaVersion:1 as const,issueId,productSha,reviewId,reviewerEnvelopePath:"reviewer.json",intentRefs:[intentPath],evidenceRefs:[acceptancePath]});
...
function fixture(entries:readonly ReturnType<typeof entry>[],overrides:Readonly<{product?:()=>Promise<string>;scope?:()=>Promise<Scope>}>= {}){
  let inventoryReads=0;
  const cited=new Set([intentPath,acceptancePath,...acceptance.items.flatMap(item=>item.evidenceRefs),"finding/f-1.txt","resolution/f-1.txt"]);
  const ledger={objectFormat:async()=>"sha1" as const,snapshot:async()=>({head,records:{}}),currentInventory:async(received:string)=>{inventoryReads++;assert.equal(received,prefix);return {head,entries};},read:async(_sha:string,paths:readonly string[])=>Object.fromEntries(paths.filter(path=>cited.has(path)).map(path=>[path,"proof"])),transact:async()=>head};
