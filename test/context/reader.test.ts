...
test("rejects a stale product SHA before it invokes the pinned ledger reader", async () => {
  let reads = 0;
  const reader = new ContextReader(expectation, authority, { currentProductSha: async () => "new-product-sha" }, { read: async () => { reads += 1; return {}; } });
...
test("rejects a ledger response that omits an allowed required record", async () => {
  const reader = new ContextReader(expectation, authority, { currentProductSha: async () => "a".repeat(40) }, { read: async () => ({ [envelope.records[0]]: "contract" }) });
...
test("rejects envelope role or delivery-id edits before product or ledger reads", async () => {
  ...
  const reader = new ContextReader(expectation, authority, { currentProductSha: async () => { productReads += 1; return envelope.productSha; } }, { read: async () => { ledgerReads += 1; return {}; } });
...
test("rejects an active binding/profile fingerprint or topology mismatch before product or ledger reads", async () => {
  for (const active of [
    ...
    let productReads = 0; let ledgerReads = 0;
    const reader = new ContextReader(expectation, { resolve: async () => active }, { currentProductSha: async () => { productReads += 1; return envelope.productSha; } }, { read: async () => { ledgerReads += 1; return {}; } });
