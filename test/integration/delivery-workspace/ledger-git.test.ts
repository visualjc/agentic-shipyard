...
test("production inventory bounds hostile record and aggregate bytes before returning entries",async()=>{const path=await repository();try{const ledger=new GitLedgerStore(path),large="x".repeat(1_000_001),head=await ledger.transact({expectedHead:undefined,writes:[{path:"deliveries/d/evidence/too-large.json",contents:large}]});await assert.rejects(ledger.currentInventory("deliveries/d/evidence/"),(error:unknown)=>error instanceof LedgerError&&error.code==="ledger-invalid-record");const second=await ledger.transact({expectedHead:head,writes:[{path:"deliveries/e/evidence/one.json",contents:"y".repeat(1_000_000)},{path:"deliveries/e/evidence/two.json",contents:"z".repeat(1_000_001)}]});assert.ok(second);await assert.rejects(ledger.currentInventory("deliveries/e/evidence/"),(error:unknown)=>error instanceof LedgerError&&error.code==="ledger-invalid-record");}finally{await rm(path,{recursive:true,force:true});}});
...
test("reads exact pinned ledger commits for ContextReader and fails closed for unavailable Git objects", async () => {
  ...
  try {
    ...
    const envelope = createEnvelope({
      ...
      productBranch: "main", objectFormat: "sha1", productSha: await git(path, ["rev-parse", "HEAD"]), ledgerRef: GitLedgerStore.ref, ledgerSha: first,evidenceManifestDigest:"c".repeat(64),
    });
    const loaded = await new ContextReader({
      ...
      productSha: envelope.productSha, ledgerRef: envelope.ledgerRef, ledgerSha: envelope.ledgerSha,evidenceManifestDigest:envelope.evidenceManifestDigest, objectFormat: envelope.objectFormat,
