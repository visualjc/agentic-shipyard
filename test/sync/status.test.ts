...
test("public status composes local sync facts without a mutation, provider, or delivery dependency", async () => {
  ...
  assert.equal(result.graphFreshness, "fresh"); assert.equal(result.destinationSha, "b".repeat(40)); assert.equal(result.providerRefs?.sourceProvenance, "none"); assert.equal(result.nextSafeAction, "Continue safely."); assert.deepEqual(result.blockers, []);
...
test("public status reports local sync blockers and next action instead of unconditional unavailable", async () => {
  ...
  const result = await status({ resolve: async () => binding }, { commonDirectory: async () => "/repo/.git", remoteUrl: async () => repository.remote.url }, { read: async () => profile }, "/repo", reader);
  assert.equal(result.graphFreshness, "stale"); assert.deepEqual(result.blockers, [{ code: "sync-dirty", message: "Worktree or index is dirty." }]); assert.equal(result.nextSafeAction, "Clean it explicitly.");
});
