...
export type EvidenceManifest = Readonly<{ schemaVersion:1; issueId: string; items: readonly Readonly<{ id: string; kind: AcceptanceItem["kind"]; notApplicableEligible?: boolean }>[] }>;
export type EvidenceDecision = Readonly<{ acceptanceFresh: boolean; reviewFresh: boolean; blockingFindingIds: readonly string[]; staleRecordIds: readonly string[]; blockers: readonly string[]; nextAction: "gather-evidence" | "resolve-finding" | "renew-acceptance-and-review" | "pin-evidence" | "proceed-to-promotion-gate"; promotionEligible: boolean }>;
