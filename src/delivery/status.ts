...
export type DeliveryStatusPins = Readonly<{ productSha?: string; ledgerSha?: string }>;
...
export function deliveryStatusContributor(pins: DeliveryStatusPins): StatusContributor {
  return () => ({ ...(pins.productSha === undefined ? {} : { productSha: pins.productSha }), ...(pins.ledgerSha === undefined ? {} : { ledgerSha: pins.ledgerSha }) });
}
