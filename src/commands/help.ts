export const HELP = {
  shipyard: "shipyard <setup|status|sync|review|help> [options] — dispatch a safe Shipyard command.",
  ...
  help: "shipyard-help [setup|status|sync|review|help] — show focused, read-only command guidance.",
  review: "shipyard-review — show the exact-SHA independent-review procedure; launch through the trusted API only.",
} as const;

export function help(topic = "shipyard"): string {
  return HELP[topic as keyof typeof HELP] ?? `Unknown help topic: ${topic}. Available topics: setup, status, sync, review, help.`;
}
