...
import type { GraphCacheLock, GraphDescriptor, GraphRuntime, GraphSource } from "./types.js";
...
export function validateGraphLock(value: unknown): GraphCacheLock { const v = plain(value, "lock"); only(v, ["ownerHost", "ownerPid", "acquiredAt"], "lock"); const ownerPid = v.ownerPid; if (typeof v.ownerHost !== "string" || typeof ownerPid !== "number" || !Number.isSafeInteger(ownerPid) || ownerPid < 1 || typeof v.acquiredAt !== "string" || !Number.isFinite(Date.parse(v.acquiredAt))) throw new GraphError("invalid-descriptor", "Invalid graph lock."); return Object.freeze({ ownerHost: v.ownerHost, ownerPid, acquiredAt: v.acquiredAt }); }
