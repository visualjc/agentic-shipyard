...
function digest(value: unknown, name: string): string { const result = string(value, name); if (!/^[0-9a-f]{64}$/.test(result)) throw new GraphError("invalid-descriptor", `Invalid ${name} graph digest.`); return result; }
...
export function validateGraphDescriptor(value: unknown): GraphDescriptor {
  const v = plain(value, "descriptor"); only(v, ["adapter", "reviewedToolSource", "artifactSha256", "runtimeArtifactSha256", "contentSha256", "cacheIdentity", "cacheRoot", "worktreeRoot", "worktreeInstanceId", "indexedCommit", "workingTreeFingerprint", "refreshedAt", "seededFromSha"], "descriptor"); const adapter = string(v.adapter, "descriptor");
  ...
  const artifactSha256 = digest(v.artifactSha256, "artifact"); const runtimeArtifactSha256 = v.runtimeArtifactSha256 === undefined ? undefined : digest(v.runtimeArtifactSha256, "runtime artifact"); const contentSha256 = digest(v.contentSha256, "content");
  if ((adapter === "graphify" && runtimeArtifactSha256 !== undefined) || (adapter === "codegraph" && runtimeArtifactSha256 === undefined)) throw new GraphError("invalid-descriptor", "Invalid graph runtime artifact binding.");
  ...
  return Object.freeze({ adapter, reviewedToolSource: receipt, artifactSha256, ...(runtimeArtifactSha256 ? { runtimeArtifactSha256 } : {}), contentSha256, cacheIdentity, cacheRoot: root(v.cacheRoot, "cache"), worktreeRoot: root(v.worktreeRoot, "worktree"), worktreeInstanceId, indexedCommit: sha(v.indexedCommit, "descriptor"), workingTreeFingerprint, refreshedAt, ...(seededFromSha ? { seededFromSha } : {}) } as GraphDescriptor);
}
