...
export interface GraphifyFiles { exists(path: string): Promise<boolean>; observeProductTree(worktreeRoot: string): Promise<unknown>; auditProductTree(worktreeRoot: string, observation: unknown): Promise<unknown>; canonicalPath(path: string): Promise<string | undefined>; contentDigest(path: string): Promise<string>; copy?(from: string, to: string): Promise<void>; remove?(path: string): Promise<void>; }
...
function productAudit(value: unknown): "clean" | "restored" | "ambiguous" | "failed" | undefined { const result = plain(value); return result && Object.keys(result).length === 1 && (result.state === "clean" || result.state === "restored" || result.state === "ambiguous" || result.state === "failed") ? result.state : undefined; }
...
async function operateGraphify(inputSource: GraphSource, inputOptions: GraphifyOptions, baseline?: GraphBaseline): Promise<{ decision: GraphDecision; descriptor?: GraphDescriptor }> {
  ...
  const outcome = await withGraphOperationLock(operationLock, cacheRoot, lockIdentity, async () => {
    ...
    let productObservation: unknown; try { productObservation = await files.observeProductTree(worktreeRoot); if (productObservation === undefined) throw new Error(); } catch { return { decision: graphDecision("failed", "Graphify product-tree observation failed safely before execution.") }; }
    let operationDecision: GraphDecision | undefined;
    if (baseline) {
      ...
      try { await files.copy(join(baseline.descriptor.cacheRoot, "graphify-out"), output); if (!await files.exists(output)) throw new Error(); }
      catch { try { await files.remove(cacheRoot); } catch {} operationDecision = graphDecision("failed", "Graphify seed verification failed."); }
    } else {
      const output = join(cacheRoot, "graphify-out"); try { const result = await command.run(binary, ["index", "--code-only", "--out", output], { cwd: worktreeRoot, env: { GRAPHIFY_OUT: output, GRAPHIFY_QUERY_LOG_DISABLE: "1" }, artifact: observation }); if (!successful(result) || !await files.exists(output)) operationDecision = graphDecision("failed", "Graphify refresh verification failed."); }
      catch { operationDecision = graphDecision("failed", "Graphify refresh verification failed."); }
    }
    let audit: ReturnType<typeof productAudit>; try { audit = productAudit(await files.auditProductTree(worktreeRoot, productObservation)); } catch { audit = undefined; }
    if (audit === "ambiguous" || audit === "failed" || audit === undefined) return { decision: graphDecision("failed", "Graphify product-tree cleanup could not be proven; preserve ambiguous paths and inspect the worktree directly.") };
    if (audit === "restored") return { decision: graphDecision("failed", "Graphify created product-worktree artifacts; invocation-owned additions were removed and the original tree was restored.") };
    if (operationDecision) return { decision: operationDecision };
