...
export class GraphLockService {
  ...
  async acquire(path: string): Promise<{ decision?: GraphDecision; lock?: GraphCacheLock }> {
    ...
    const lock: GraphCacheLock = Object.freeze({ ownerHost: this.process.hostName(), ownerPid: this.process.processId(), acquiredAt: this.process.now().toISOString() });
  ...
  async release(path: string, lock: GraphCacheLock): Promise<void> {
    if (!await this.store.removeVerified(path, lock)) throw new Error("Graph lock release could not be verified.");
  }
